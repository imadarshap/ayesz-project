...
class ImportExcelController extends Controller
{
    ...
    public function import(Request $request)
    {
        ...
        while ($csv_line = fgetcsv($fp, 1024)) {
            ...
            for ($i = 0, $j = count($csv_line); $i < $j; $i++) {
                ...
                $vendor_ids = isset($csv_line[8]) ? $csv_line[8] : '';
            }
            $i++;
            $check_cat = DB::table('categories')->where('cat_id', $insert_csv0['cat_id'])->first();
            if (empty($check_cat)) {
                fclose($fp) or die("can't close file");
                return back()->withErrors('Invalid Category Id - ' . $insert_csv0['cat_id']);
            } else {
                ...
                $varient_id = DB::table('product_varient')->insertGetId($data1);

                if (!empty($vendor_ids)) {
                    $vendors = explode(',', $vendor_ids);
                    foreach ($vendors as $vendor_id) {
                        $store = DB::table('store')->where('store_id', trim($vendor_id))->first();
                        if (!empty($store)) {
                            $insert2 = DB::table('store_products')
                                ->insert(['store_id' => $store->store_id, 'stock' => 0, 'varient_id' => $varient_id, 'price' => $insert_csv1['base_price'], 'mrp' => $insert_csv1['base_mrp'], 'stock' => 0]);
                        }
                    }
                }
            }
    ...
    public function import_varients(Request $request)
    {
        ...
        $admin = DB::table('admin')
            ->where('admin_email', $admin_email)
            ->first();
        if(!Helper::hasRight($admin->id,'varients','Add')){
            return abort(403);
        }
        $this->validate(
            $request,
