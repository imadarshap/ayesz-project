...
class DriverorderController extends Controller
{
	...
	public function accept_order(Request $request){
    	...
    	if($status=="Accepted_By_Delivery_Agent"){
        	...
        	$message = array('status'=>'1', 'message'=>'order is accepted');
			return $message;
        }else{
    		...
    		if($del_boy){
            	...
            	$orderconfirm = DB::table('orders')
                   	->where('cart_id',$cart_id)
                   	->update(['order_status'=>'Rejected_By_Delivery_Agent',
                   	'dboy_id'=>0]);
      ...
      foreach($ord as $ords){
      	...
      	$details  =   DB::table('store_orders')
    	               ...
    	               ->where('store_approval',1)
        				->get();
         $total_items  =   DB::table('store_orders')
    	               ->where('order_cart_id',$cart_id)
    	               ->where('store_approval',1)
