...
public function storeorders(Request $request)
     {
      ...
      foreach($ord as $ords){
        ...
        $data[]=array('user_address'=>$ords->house_no.','.$ords->society.','.$ords->city.','.$ords->landmark.','.$ords->state.','.$ords->pincode,
                      'cart_id'=>$cart_id,
      ...
      if(count($cartss)>0){
       ...
       $p = DB::table('store_products')
        ...
        $carte= DB::table('store_orders')
            ...
            ->get();
        $user = DB::table('users')
                     ->where('user_id',$ordr->user_id)
                     ->first();
        $orderplacedmsg = $this->orderreject($ordr->cart_id,$prod_name,$price2,$user->user_phone,'Vendor');
        $data[]=array('result'=>'Order Rejected successfully');
        return $data;
